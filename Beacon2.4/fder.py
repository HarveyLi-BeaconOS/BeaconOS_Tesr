...
class configure_path:
    def init(file_name: str | None = "err_exp_en_us.json", lang: str | None = "en_us") -> str | bool:
        configured_path = str(Path("lang").absolute()) + "/" + lang + "/" +file_name
...
class load_js:
    def init(path: str | bytes | None = ...) -> dict | bool:
        if exists(path):
...
def fder(object: str | int | None = ..., lang: str | None = "en_us" ,flag: str | None = ...):
