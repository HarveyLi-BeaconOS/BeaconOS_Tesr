...
def print_error(error_file_name: str | None = None, error_name: str | None = None, error_ln: int | None = None, 
error_class: str | None = None, error_func: str | None = None, define_class_ln: int | None = None, 
define_func_ln: int | None = None,  define_file_name: str | None = None,
define_main_routine_name: str | None = None,define_class_name: str | None = "Not Classed", 
define_function_name: str | None = "No function wraped", exit_code: int | None = 1, error_details: str | None = None):
    '''
...
class executable:
    def fetch_path(config_file: str | None = "sys_config.json") -> dict | bool:
        if executable.search(destination=executable.get_addr(file_name=config_file),filename=config_file):
            with open(executable.get_addr(file_name=config_file),'r') as file:
                config = json.load(file)
            return config
        else:
            return False
    
    def get_exec_path(config: dict | None = ...) -> str | None:
        if len(config) <= 0:
            return None
        else:
...
def cmt(what: int | None = 1,word_source: list | str | None = DEFAULT_SOURCE_WORD,
sentence_source: list | str | None = DEFAULT_SOURCE_SENTENCE_EMPTY, init: bool | None = False) -> None:
    if not word_source in DEFAULT_SOURCE_WORD or not sentence_source in DEFAULT_SOURCE_SENTENCE_EMPTY:
        if what == 1:
            print(random.choice(word_source))
        else:
            print(random.choice(word_source))
    else:
        if what == 1:
            ...
            print(random.choice(word_source))
    number_register.append(1)
    if len(number_register) == 10:
        print("Dude, you are using this command",len(number_register),"times.")
    if len(number_register) == 50:
        print(print("Dude, ",len(number_register),"times???? Stop!"))
